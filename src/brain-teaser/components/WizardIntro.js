...
export default function WizardIntro({renderIntroOver,description}) {
    return (
        <Card
        ...
        <p className={styles.wizard_intro_text}> {description} In the given time try to memorize the words and their order then 
            they get unorderd and you gonna get them back tidy
        </p>
        <Button className={styles.wizard_Btn} onClick={()=>renderIntroOver(false)} text={'GET GOING'}/>
                 </div>
